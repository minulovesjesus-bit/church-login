  ...
  return (
    <main className="teacher-events-shell">
      <header className="teacher-events-header">
        ...
        <button className="secondary-button" type="button" disabled={mutationPending} onClick={refresh}>새로고침</button>
      </header>
        ...
        <section className="teacher-event-list-card" aria-labelledby="event-list-title">
          ...
          {response ? (
            <nav className="teacher-event-pagination" aria-label="일정 페이지">
              <button className="secondary-button" type="button" disabled={mutationPending || page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>이전 페이지</button>
              <span>{page} / {totalPages} 페이지 · 총 {response.total}건</span>
              <button className="secondary-button" type="button" disabled={mutationPending || page >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>다음 페이지</button>
            </nav>
          ) : null}
        </section>
      </div>
